...
// Create the crc32 table, user needs to provide a uint32_table crc32_table[256] buffer.
// The table is already created below (indra_crc32_table)
// g(x) = x32 + x26 + x23 + x22 + x16 + x12 + x11 + x10 + x8 + x7 + x5 + x4 + x2 + x + 1
// 0xEDB88320 is the so-called "reversed" representation of the CRC32 generator polynomial.
...
// c.f.: "Todd K. Moon, "Error Correction Coding. Mathematical Methods and Algorithms", Wiley, 2005, ISBN 0-471-64800-0"
// Impl: https://lxp32.github.io/docs/a-simple-example-crc32-calculation/
...
int iaCrc32Selftest() {
  ...
  uint32_t crc = iaCrc32((unsigned char *)test_string, 9);
  uint32_t exp_crc=0xCBF43926;
  if (crc == exp_crc) return 1;
  return 0;
}
